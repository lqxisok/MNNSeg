...
public class CommandBufferTestControlTemp : MonoBehaviour {
    ...
    void Start()
    {
        m_cameraRT = new RenderTexture(Screen.width, Screen.height, 0);
        SourceCamera.targetTexture = m_cameraRT;
        oldOrientation = SourceCamera.transform.localRotation;
    ...
    private void Update()
    {
        ...
        Vector3 delta = (SourceCamera.transform.localRotation * Quaternion.Inverse(oldOrientation)).eulerAngles;
        if (delta.x > 180) delta.x -= 360;
        if (delta.y > 180) delta.y -= 360;
        if (delta.z > 180) delta.z -= 360;
        ARBackgroundMat.SetVector("_Vec", delta);
